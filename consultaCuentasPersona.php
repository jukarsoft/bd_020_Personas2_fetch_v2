...
//conexion bbdd Personas
	...
	try {
		...
		//SELECT 
			//WHERE    
		// ORDER BY
		$sql="SELECT cuentas.* FROM personascuentas INNER JOIN cuentas ON personascuentas.idcuentas = cuentas.idcuentas WHERE personascuentas.pk_personas = $pk";
		$resultado=mysqli_query ($bd, $sql);
		if (mysqli_errno($bd)!=0) {
			throw new Exception('<<error obtención de la cuentas>>',mysqli_errno($bd));			
		} 
		$cuentas = array();
